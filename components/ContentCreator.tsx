    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        switch (activeSubTab) {
            case 'card-news':
                ...
                prompt = `# AI 카드뉴스 제작 요청\n## 주제: ${currentFormData.topic}\n## 핵심 타겟: ${currentFormData.targetAudience}\n## 필수 포함 정보: ${currentFormData.requiredInfo}\n## 요청 사항: 위 정보를 바탕으로, 인스타그램에 바로 올릴 수 있는 카드뉴스 콘텐츠를 10장 이내로 제작해주세요. 각 장의 내용을 명확히 구분하고, 이모지를 활용하여 가독성을 높여주세요. 마크다운 문법은 사용하지 마세요.\n\n[1번 카드]\n제목...\n\n[2번 카드]\n내용...`;
                break;
            case 'blog':
                currentFormData = { ...blogFormData };
                title = `블로그: ${currentFormData.topic}`;
                prompt = `# AI 블로그 포스팅 작성 요청\n## 주제: ${currentFormData.topic}\n## 핵심 키워드: ${currentFormData.keywords}\n## 핵심 타겟: ${currentFormData.targetAudience}\n## 글의 톤앤매너: ${currentFormData.tone}\n## 요청 사항: 위 정보를 바탕으로, 약국 블로그에 게시할 전문적인 정보성 포스팅을 작성해주세요. SEO(검색엔진 최적화)를 고려하여 키워드를 자연스럽게 본문에 녹여내고, 독자의 흥미를 유발할 수 있는 도입부와 명확한 결론을 포함해주세요. 마크다운 문법은 사용하지 말고, 자연스러운 단락과 소제목으로 구성해주세요.`;
                break;
            case 'video':
