...
const ChatModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50 p-4 transition-opacity duration-300" onClick={closeChat}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl h-full max-h-[90vh] sm:max-h-[700px] flex flex-col transition-transform duration-300" onClick={(e) => e.stopPropagation()}>
          ...
          {chatHistory.map((msg) => (
            <div key={msg.id} className={`flex items-start gap-3 w-full ${msg.role === 'user' ? 'justify-end' : ''}`}>
                {msg.role === 'model' && (
                 ...
                 <div className={`max-w-lg p-3 rounded-xl whitespace-pre-wrap text-sm break-words ${
                      msg.role === 'user'
        ...
        <footer className="p-4 border-t border-slate-200 flex-shrink-0">
          <form onSubmit={handleSubmit} className="flex items-center space-x-2 sm:space-x-3">
            ...
            <input
              ...
              className="flex-grow px-4 py-2 bg-slate-100 border border-transparent rounded-full focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-shadow text-sm sm:text-base"
